"""
Compatibility wrapper for running SpecForge in the SGLang ROCm Docker image.
...
try:
    ...
    # Provide stub functions - these are only used in the custom backend
    # which we won't actually call (we use sglang backend)
...
if not hasattr(PreTrainedModel, "all_tied_weights_keys"):
    ...
    print("[compat] Added all_tied_weights_keys property to PreTrainedModel")

# Now import SpecForge (which triggers all the sub-imports)
# Then patch SGLang function signatures
...
def _patched_to_kwargs(self):
    ...
    # Force max_total_tokens to ensure enough KV cache for 16K context
    if "max_total_tokens" in server_args_params:
        filtered["max_total_tokens"] = 48000
        print("[compat] Set max_total_tokens=32000")
    return filtered
...
sys.argv[0] = "/workspace/SpecForge/scripts/train_eagle3.py"
runpy.run_path("/workspace/SpecForge/scripts/train_eagle3.py", run_name="__main__")
