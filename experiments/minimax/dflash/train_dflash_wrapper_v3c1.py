...
def _patched_to_kwargs(self):
    ...
    if "max_total_tokens" in server_args_params:
        filtered["max_total_tokens"] = 8192
        print("[compat] Set max_total_tokens=8192")
    filtered["mem_fraction_static"] = 0.95
    print(f"[compat] Forced mem_fraction_static=0.95")
...
# Patch scheduler to be constant LR (no decay)
import specforge.optimizer as _opt
_OrigBF16Optimizer = _opt.BF16Optimizer
class ConstantLRBF16Optimizer(_OrigBF16Optimizer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Replace cosine scheduler with constant LR
        self.scheduler = torch.optim.lr_scheduler.LambdaLR(self.optimizer, lr_lambda=lambda step: 1.0)
        print(f"[compat] Using constant LR = {self.optimizer.param_groups[0]['lr']}")
import torch
_opt.BF16Optimizer = ConstantLRBF16Optimizer
print("[compat] Patched BF16Optimizer for constant LR")
