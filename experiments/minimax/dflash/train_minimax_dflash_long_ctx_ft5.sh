...
# MiniMax-M2.7 DFlash long-ctx ft5
# Init from best ft2 step 47k, constant LR 1e-5
# Mixed 41k data (minimax 25k + kimi regen 16k), minimax eval
...
torchrun \
    ...
    --draft-config-path /workspace/checkpoints/dflash/minimax-m2.7-long-ctx-ft5/epoch_0_step_0/config.json \
    --train-data-path /workspace/data/kimi-k2.6-eagle3-data/train_mixed_41k.jsonl \
    --eval-data-path /workspace/data/minimax2.7-spec-data/synthesis_25k_eval.jsonl \
    --build-dataset-num-proc 96 \
    --output-dir /workspace/checkpoints/dflash/minimax-m2.7-long-ctx-ft5/ \
    --tp-size $TP_SIZE \
